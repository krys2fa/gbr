...
export function middleware(req: NextRequest) {
  ...
  const cookie = req.cookies.get('next-auth.session-token')?.value || req.cookies.get('__Secure-next-auth.session-token')?.value;
  if (cookie) {
  ...
  if (!role || !rule.roles.includes(role)) {
    return new NextResponse(JSON.stringify({ error: 'Forbidden: insufficient role' }), { status: 403, headers: { 'Content-Type': 'application/json' } });
  }
